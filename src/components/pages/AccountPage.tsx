...
export function AccountPage({ products }: { products: Product[] }) {
  ...
  const tabs = [
    { id: 'profile' as const,  label: lang === 'fr' ? 'Profil' : 'الملف الشخصي' },
    { id: 'orders' as const,   label: lang === 'fr' ? 'Commandes' : 'طلباتي' },
    { id: 'wishlist' as const, label: lang === 'fr' ? 'Favoris' : 'مفضلاتي', count: fav.length },
  ];
